...
def build_remember_insight_tool(
    ...
    @tool(
        ...
        description=(
            ...
            "patterns, goal for intentions, learning or interest for durable study topics, education or "
            "schedule for syllabus/course due dates, communication for contact-channel facts, personality for explicit "
            "personality or communication-style facts, assessment for official tests or evaluations, profile "
            "for durable background facts, shopping for merchant or purchase patterns, and context "
            "for other durable facts. Receipts and order confirmations are evidence for a shopping memory, "
            "not a reason to create one memory per transaction. When repeated dated purchases support it, "
            "record the last observed date and an approximate cadence (for example weekly, monthly, or "
            "about every six months) under one stable key; do not infer a preference from one isolated "
            "purchase. The same rule applies to recurring grocery shopping, delivery services, household "
            "purchases, and other repeat needs. Search related historical "
