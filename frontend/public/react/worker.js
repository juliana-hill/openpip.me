...
function parseVoiceStatesBin(buffer) {
  ...
  for (let v = 0; v < voiceCount; v++) {
    ...
    offset += 2;
    const name2 = new TextDecoder().decode(new Uint8Array(buffer, offset, nameLen));
    offset += nameLen;
...
function stem(name2) {
  return config.quantized ? `${name2}_int8.onnx` : `${name2}.onnx`;
}
...
function groupVoiceRecordByModule(record) {
  ...
  for (const [key, value] of Object.entries(record)) {
    ...
    if (slash === -1) continue;
    const moduleName2 = key.slice(0, slash);
    const tensorKey = key.slice(slash + 1);
    if (!grouped[moduleName2]) grouped[moduleName2] = {};
    grouped[moduleName2][tensorKey] = value;
  }
  return grouped;
}
...
async function createSession(language, name2, onProgress) {
  const bytes = await fetchWithProgress(modelUrl(language, stem(name2)), name2, onProgress);
...
async function init(cfg) {
  ...
  const sessions = {};
  for (const name2 of needed) {
    sessions[name2] = await createSession(language, name2, onProgress);
  }
...
async function loadBuiltinVoice(name2) {
  await ensureVoicesBin();
  if (!predefinedVoiceRecords[name2]) throw new Error(`Unknown built-in voice: ${name2}`);
  const ref = `builtin:${name2}`;
  if (!voiceStateCache.has(ref)) {
    voiceStateCache.set(ref, stateFromVoiceRecord(predefinedVoiceRecords[name2]));
  }
  return ref;
}
  ...
  for (let chunkIdx = 0; chunkIdx < chunks.length; chunkIdx++) {
    ...
    for (let step = 0; step < MAX_FRAMES; step++) {
      ...
      for (let lsdIndex = 0; lsdIndex < LSD_STEPS; lsdIndex++) {
        ...
        for (let i = 0; i < latentDim; i++) latentData[i] += flowDir[i] * dt2;
      }
...
self.onmessage = async (e) => {
  const { id: id2, type, payload } = e.data;
  try {
    if (type === "init") {
      await init(payload);
      post({ id: id2, type: "result", result: { ok: true } });
    } else if (type === "cloneVoice") {
      const ref = await cloneVoice(payload.audio, payload.ref);
      post({ id: id2, type: "result", result: { ref } });
      ...
      post({ id: id2, type: "result", result: { metrics } });
    } else if (type === "stop") {
      isGenerating = false;
      post({ id: id2, type: "result", result: { ok: true } });
    }
    ...
    isGenerating = false;
    post({ id: id2, type: "error", error: err && err.message ? err.message : String(err) });
  }
};
