...
// compat/pocket-tts.ts
var PocketTTS = class {
  constructor(_options) {
  }
  async load(_onProgress) {
  }
  async loadVoice(_voice) {
    return "";
  }
  async stop() {
  }
  ...
  async generate(_text, _options) {
    return { stopped: true, audioDuration: 0 };
  }
};
...
var POCKET_TTS_SAMPLE_RATE = 24e3;
var naturalEnginePromise = null;
var naturalEngine = null;
...
function stopNaturalPlayback() {
  ...
  activePlayer?.stop();
  activePlayer = null;
  void naturalEngine?.tts.stop().catch(() => void 0);
}
async function getNaturalEngine(callbacks) {
...
async function speakWithSystemVoice(text7, callbacks) {
  if (!("speechSynthesis" in window)) return false;
  try {
    const synth = window.speechSynthesis;
    callbacks.onStatus?.("fallback");
    const utterance = new SpeechSynthesisUtterance(text7);
...
async function speakNaturally(text7, callbacks = {}) {
  if (typeof window === "undefined" || !text7.trim()) return false;
  stopSpeaking();
  const run = activeRun;
  ...
  try {
    ...
    if (availability === "unavailable") {
      await player.destroy();
      return speakWithSystemVoice(text7, callbacks);
    }
    ...
    const metrics = await engine.tts.generate(text7, {
      voice: engine.voice,
      onChunk: (audio, meta) => {
    ...
    finishTimer = window.setTimeout(() => {
      ...
      finishTimer = null;
      callbacks.onEnd?.();
    }, Math.max(150, Math.ceil(metrics.audioDuration * 1e3)));
    return true;
  } catch (error) {
    if (run === activeRun) {
      activePlayer?.stop();
