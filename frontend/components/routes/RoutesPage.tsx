...
type AgentPipelineOutput = {
  ...
  stays?: Array<{ name?: string; neighborhood?: string; area?: string; type?: string; description?: string; detail?: string; safety?: string; sourceUrl?: string | null }>;
  places?: Array<{ name?: string; type?: string; description?: string; detail?: string; route?: string; sourceUrl?: string | null }>;
...
function itineraryBulletSummary(value: unknown): string {
...
function ItineraryDay({ date, title, detail, icon: Icon }: { date: string; title: string; detail: string; icon: typeof Plane }) {
  return <div className={styles.dayRow}><div className={styles.dayRail}><span aria-label={date}><Icon size={17} aria-hidden="true" /></span><i /></div><div className={styles.dayBody}><div className={styles.dayHeading}><div><span className={styles.dayDate}>{date}</span><h3>{itineraryDayTitle(title)}</h3></div></div><AgentMarkdown className={styles.dayDescription} value={itineraryBulletSummary(detail)} /></div></div>;
}
...
const genericPreparationTitles = new Set(["item", "preparation", "preparation item", "what you'll need", "what you’ll need"]);

function preparationTitle(item: { title?: string; detail?: string }): string {
  const title = item.title?.trim() || "";
  if (title && !genericPreparationTitles.has(title.toLowerCase())) return title;
  return item.detail?.trim().split(/[.;]/, 1)[0]?.trim() || "";
}

function itineraryDayTitle(value?: string): string {
...
function AgentMarkdown({ value, className }: { value: unknown; className?: string }) {
  const text = readableAgentText(value);
  if (!text) return null;
  return <Markdown className={className} remarkPlugins={[remarkGfm]}>{text}</Markdown>;
}
...
function ResearchRecommendations({ output }: { output: AgentPipelineOutput }) {
  ...
  return <div className={styles.recommendationGrid}>
    {stays.length > 0 && <Card className={styles.recommendationCard}><CardHeader className={styles.sectionHeader}><div><p className={styles.eyebrow}>Places to stay</p><CardTitle>Suggested bases</CardTitle></div><Compass size={19} className={styles.mutedIcon} /></CardHeader><CardContent className={styles.recommendationList}>{stays.map((stay, index) => <div className={`${styles.recommendationRow} ${styles.stayRecommendationRow}`} key={`${stay.name}-${index}`}><div className={styles.stayTile}><Building2 size={22} aria-hidden="true" /></div><div className={styles.stayRecommendationContent}><div className={styles.placeRecommendationTitle}><strong>{stay.name}</strong><span className={styles.recommendationType}>{placeTypeLabel(stay.type)}</span></div><span>{stay.neighborhood || stay.area}</span><AgentMarkdown className={styles.recommendationMarkdown} value={stay.description || stay.detail} />{stay.safety && <AgentMarkdown className={styles.recommendationNote} value={stay.safety} />}</div>{stay.sourceUrl && <a className={styles.recommendationSourceLink} href={stay.sourceUrl} target="_blank" rel="noopener noreferrer">View source <ExternalLink size={12} /></a>}</div>)}</CardContent></Card>}
    {places.length > 0 && <Card className={styles.recommendationCard}><CardHeader className={styles.sectionHeader}><div><p className={styles.eyebrow}>What to see</p><CardTitle>Places and activities</CardTitle></div><MapPin size={19} className={styles.mutedIcon} /></CardHeader><CardContent className={styles.recommendationList}>{places.map((place, index) => { const PlaceIcon = placeIconForType(place.type); return <div className={`${styles.recommendationRow} ${styles.placeRecommendationRow}`} key={`${place.name}-${index}`}><span className={styles.placeRecommendationIcon}><PlaceIcon size={26} aria-hidden="true" /></span><div className={styles.placeRecommendationContent}><div className={styles.placeRecommendationTitle}><strong>{place.name}</strong><span className={styles.recommendationType}>{placeTypeLabel(place.type)}</span></div><AgentMarkdown className={styles.recommendationMarkdown} value={place.description || place.detail} />{place.route && <AgentMarkdown className={styles.recommendationNote} value={place.route} />}{place.sourceUrl && <a className={styles.recommendationSourceLink} href={place.sourceUrl} target="_blank" rel="noopener noreferrer">View source <ExternalLink size={12} /></a>}</div></div>; })}</CardContent></Card>}
  </div>;
}
  ...
  return <div className={styles.planBuildPage} aria-live="polite">
    <Card className={styles.planBuildCard}>
      <CardContent>
        ...
        {failed && onRetry && <Button variant="secondary" onClick={onRetry}>Continue research</Button>}
    ...
    : routeTripId && !draft
      ...
      : draft
        ? planBuilding || planBuildError
          ? <PlanBuildScreen draft={draft} agentName={agentName} stage={planBuildStage} error={planBuildError} onRetry={() => retryPlanBuild(false)} onBack={showOverview} />
